 ...
 *
 * 背景：噜噜 / 肥嘟袋鼠 用的是 `public/characters/*.png` 位图立绘，
 * 狼影 / 蜂针 / 磐垒 用的是 `art.ts` 里的矢量小人。
...
describe('新增角色的立绘登记', () => {
  ...
  test('原有三个角色没有 sprite（继续走矢量渲染，不受位图影响）', () => {
    for (const id of ['wolfshade', 'sting', 'bulwark']) {
  ...
  test('四个位图角色的宽高比都在合理区间（高度统一 → 视觉大小一致）', () => {
    // 与 public/characters/*.png 实际像素尺寸对应的宽高比
    ...
    for (const [slug, aspect] of Object.entries(aspects)) {
